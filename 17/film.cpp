...
void saveFilmsToFile(const Film* films, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        outFile << "Name: " << films[i].title << endl;
        outFile << "Date: " << films[i].year << endl;
        outFile << "--------------------" << endl;
    }
    outFile.close();
    cout << "Films saved to Films.txt successfully!" << endl;
}
